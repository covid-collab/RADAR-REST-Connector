...
public class RestTaskTest {
  ...
  public void restTest(WireMockRule wireMock) throws InterruptedException {
    ...
    assertEquals(String.class, messages.get(0).value().getClass());
    assertEquals(RESPONSE_BODY, messages.get(0).value());
    assertEquals(TOPIC, messages.get(0).topic());
    ...
    assertEquals(1, messages.size());
    assertEquals(byte[].class, messages.get(0).value().getClass());
    assertEquals(RESPONSE_BODY, new String((byte[]) messages.get(0).value()));
    assertEquals(TOPIC, messages.get(0).topic());
